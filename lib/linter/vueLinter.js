...
module.exports = async function (source, path, config) {
  ...
  if (descriptor.template && descriptor.template.lang === "pug") {
    const result = await pugLinter(descriptor.template.content, path, config);
    result.messages && messages.push(...result.messages);
  }

  // vue 文件需要提取 script 内容
  if (descriptor.script) {
    const result = await jsLinter(descriptor.script.content, path, config);
    result.messages && messages.push(...result.messages);
  }
  ...
  if (descriptor.styles.length > 0) {
    for (const style of descriptor.styles) {
      const result = await jsLinter(style.content, path, config);
      result.messages && messages.push(...result.messages);
    }
  }
